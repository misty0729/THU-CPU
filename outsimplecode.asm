...
NOP
LI R5 8                        // 判断当前按键是否为可见字符 d是enter
SLTU R1 R5
BTEQZ E                         // 当前按键不是可见字符，跳至 JUDGEBKSP 
NOP
LI R4 4F                        // 当前按键是可见字符，判断当前光标位置是否到达本行行尾
CMP R3 R4
BTEQZG ETCURRENTKEY             // 如果到达行尾，不写入当前字符，跳至 GETCURRENTKEY 继续读取按键
NOP
ADDIU R3 1                      // 当前光标列号 +1
LI R5 BF                        // 写入当前字符
SLL R5 R5 0
...
NOP
JUDGEBKSP: LI R5 8            // 判断当前按键是否为backspace
SUBU R5 R1 R5
